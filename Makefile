...
.PHONY: kind
kind:
	kind get clusters -q | grep "skeletonpy" || kind create cluster --config kind.yaml

.PHONY: clean
clean:
	kind delete cluster --name skeletonpy
...
.PHONY: grpc
grpc:
	grpcurl -protoset <(buf build -o -) -plaintext -d '{"name": "alice"}' localhost:55050 skeletonpy.greeter.v1.Greeter/Hello || true
